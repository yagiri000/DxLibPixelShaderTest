...
public:
	...
	//点と矩形
	static bool BoxPoint(int px, int py, int box_cx, int box_cy, int box_width, int box_height){
	...
	*/
	...
	static double sinsec(double base, double a, double t, double frame){
...
{
public:
	int x, y, width, height, col;//ボタンの大きさ
	std::string label;//ボタンの名称
	std::function<void(void)> func;
	bool deleteflag;

	IButton(int x_, int y_, int width_, int height_, std::string label_, std::function<void(void)> func_){
		x = x_;
		y = y_;
		width = width_;
		height = height_;
...
public:
	...
	OnOffButton(int x_, int y_, int width_, int height_, std::string label_, std::function<void(void)> func_) :
		IButton(x_, y_, width_, height_, label_, func_)
	{
		IsOn = false;
	}
	...
	void CheckMouse(){
		if (Func::BoxPoint(Input::mousex(), Input::mousey(), x + width / 2, y + height / 2, width, height) && Input::keydown(250)){
			if (IsOn){
				col = 0x440000;
				IsOn = false;
			}
	...
	void draw(){
		Draw_::box(x, y, x + width, y + height, col, 255);
		...
		Draw_::centertext(x + width / 2, y + height / 2 - 10, 0xFFFFFF, Font::handle("SegoeUI20"), str, 255);
	}
};


class CrickButton : public IButton
{
public:

	CrickButton(int x_, int y_, int width_, int height_, std::string label_, std::function<void(void)> func_) :
		IButton(x_, y_, width_, height_, label_, func_)
	{
		col = 0x888888;
	}
